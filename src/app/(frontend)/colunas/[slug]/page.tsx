...
export default async function ColunaPage({ params: paramsPromise }: Args) {
  ...
  const url = '/colunas/' + decodedSlug
  const post = await queryColumnBySlug({ slug: decodedSlug })
      ...
      <div className="container max-w-[48rem] mx-auto">
        ...
        {post.relatedPosts && post.relatedPosts.length > 0 && (
          <div className="mt-12 pt-8 border-t border-border">
            <h3 className="text-base font-semibold text-foreground mb-6">Leia também</h3>
            <RelatedPosts
              className="!p-0"
        ...
        <Comments postId={String(post.id)} postType="colunas" />
      </div>
    </article>
  )
}
