...
export const HeaderClient: React.FC<HeaderClientProps> = ({ data }) => {
  ...
  return (
    <header
      className="sticky top-0 z-50 w-full border-b border-blue-700 bg-blue-700 text-white backdrop-blur-sm"
      {...(theme ? { 'data-theme': theme } : {})}
    >
      <div className="container flex h-14 items-center justify-between">
        <div className="flex items-center gap-6">
          <Link href="/" className="flex items-center gap-1.5">
            <span className="text-xl font-extrabold tracking-tight">DIAL</span>
            <span className="text-[10px] font-semibold tracking-[0.25em] uppercase">RADIO</span>
          </Link>
          <HeaderNav data={data} />
        </div>
